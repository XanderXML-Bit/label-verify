...
| **Auto-pair batches?** | Yes — drop N label images + their application files together, no manifest needed. **Two-stage pairing**: filename stem first (face-tag and app-tag aware), then **content-based fallback** (brand + class similarity from a lightweight vision extraction) for any image + app still unpaired. Handles completely randomly-named files. |
...
**Or upload a batch:** drop multiple images and their application files in one shot. **No manifest required** — the server pairs each image with its matching application file using a **two-stage strategy**:

1. **Filename stem matching** (cheap, deterministic): case-insensitive, face-tag-aware (`123-front.jpg` ↔ `123-back.jpg` ↔ `123.pdf`), app-tag-aware (`123-front.jpg` ↔ `123-app.pdf`).
2. **Content-based fallback** for anything still unpaired after filename matching. Parses each unpaired application file (brand + class + ABV) and runs a lightweight vision extraction on each unpaired image; greedy-matches by weighted similarity (brand 0.65 + class 0.25 + ABV 0.10) with a 0.55 threshold. **This handles completely randomly-named files** — a folder of `DSC_001.jpg` images and `app1.pdf`/`app2.pdf`/… apps will still get matched by their content.

The UI shows a "Detected N images + M application files" summary card before any vision call fires; content-paired rows surface in the response with `source: "content"` and a similarity score so the operator can verify before committing. Results stream back over SSE; download as JSON or CSV. Explicit manifests are still accepted in a collapsed override panel for edge cases where neither filename nor content pairing apply.
